...
module.exports = handle = (client, Client) => {
    try {
        ...
        Client.cmd.on('tebakgambar', async (data) => {
			...
			Client.reply(data.from,"Waktu tersisa 20 detik\n\nhttps://wa.me/c/6289630985223", send)
			await sleep(10000)
			if (global.tebakgambar[data.from].id != ses) return
			Client.reply(data.from,"Waktu tersisa 10 detik\n\nhttps://wa.me/c/6289630985223", send)
			await sleep(10000)
			if (global.tebakgambar[data.from].id != ses) return
			Client.reply(data.from, "Waktu habis\n\nhttps://wa.me/c/6289630985223", send)
			Client.reply(data.from,`Jawabannya adalah: ${getSoal.data.result.jawaban}\n\nhttps://wa.me/c/6289630985223`, send)
        ...
        Client.cmd.on('join', async (data) => {
            ...
            Client.acceptInviteLink(data.body).then(() => data.reply('Berhasil join\n\nhttps://wa.me/c/6289630985223')).catch(() => data.reply('Berhasil join\n\nhttps://wa.me/c/6289630985223'))
        })
        ...
        Client.cmd.on('premium', async (data) => {
            ...
            if(data.args[0].toLowerCase() == 'add') {
                if(data.args.length < 2) return data.reply('siapa?\n\nhttps://wa.me/c/6289630985223')
        ...
        Client.cmd.on('afk', (data) => {
	     ...
	     Client.cmd.on('welcome', (data) => {
            ...
            if(!data.isOwner) return data.reply(mess.admin)
	    ...
	    Client.cmd.on('youtubedl', async (data) =>{
			...
			let po = client.prepareMessageFromContent(data.from, {
                  ...
                  "description": `*hasil untuk* : ${data.body}*\n*Download vidio dengan menekan tersebut*`,
                  "buttonText": "Result",
                  "listType": "SINGLE_SELECT",
                  "sections": secs}}, {}) 
            client.relayWAMessage(po, {waitForAck: true})	
...
`
        ...
        Client.cmd.on('setgroupicon', async (data) => {
            ...
            if(!data.isAdmin) return data.reply(mess.admin)
            if(!data.botIsAdmin) return data.reply(mess.botAdmin)
            if(!data.isQuotedImage && data.type != 'imageMessage') return data.reply(`format salah!, silahkan kirim image dengan caption ${data.prefix}setgroupicon, atau reaply image dengan ${data.prefix}setgroupicon`)
            const getbuff = data.isQuotedImage ? JSON.parse(JSON.stringify(data.message).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : data.message
            const dlfile = await client.downloadMediaMessage(getbuff)
        ...
        Client.cmd.on('promote', async (data) => {
            ...
            if(!data.isAdmin) return data.reply(mess.admin)
            if(!data.botIsAdmin) return data.reply(mess.botAdmin)
            if(data.mentionedJidList.length == 0) return data.reply(`Kirim perintah *${data.prefix}${data.command} [ @tag ]*\nContoh : ${data.prefix}${data.command} @628983755600`)
        ...
        Client.cmd.on('demote', async (data) => {
            ...
            if(!data.isAdmin) return data.reply(mess.admin)
            if(!data.botIsAdmin) return data.reply(mess.botAdmin)
            if(data.mentionedJidList.length == 0) return data.reply(`Kirim perintah *${data.prefix}${data.command} [ @tag ]*\nContoh : ${data.prefix}${data.command} @6289630985223`)
        ...
        Client.cmd.on('kick', async (data) => {
            ...
            if(!data.isAdmin) return data.reply(mess.admin)
            if(!data.botIsAdmin) return data.reply(mess.botAdmin)
            if(data.mentionedJidList.length == 0) return data.reply(`Kirim perintah *${data.prefix}${data.command} [ @tag ]*\nContoh : ${data.prefix}${data.command} @6289512330535`)
            data.mentionedJidList.forEach(async jid =>{ client.groupRemove(data.from, [jid]).then(x => data.reply(`Sukses kick @${jid.split('@')[0]}`)).catch(x => data.reply(`Gagal kick @${jid.split('@')[0]}\n\nhttps://wa.me/c/6289630985223`)); await sleep(2000)})
        })
        Client.cmd.on('add', async (data) => {
            ...
            client.groupAdd(data.from, args).then(() => data.reply(`Berhasil menambahkan @${data.args.join(' @')}`)).catch(() => data.reply('Tidak dapat di tambahkan\n\nhttps://wa.me/c/6289630985223'))
        })
        ...
        Client.cmd.on('missing', async (data) => {
            ...
            if(data.isQuotedImage || data.type == 'imageMessage') {
                ...
                Client.sendFileFromBase64(data.from, getAxios.data.toString('base64'), 'missing.jpg', '*Gambar berhasil dibuat!*\n\nhttps://shiroweb-dashboard.221447.repl.co', data.message)
                ...
                Client.sendFileFromUrl(data.from, `${configs.apiUrl}/api/missing-image?apikey=${configs.zeksKey}&image=${encodeURIComponent(ppUrl)}&text1=${text[0]}&text2=${text[1]}&text3=${text[2]}`, 'follow ig 22.1447.jpg', '*Gambar berhasil dibuat!*\n\nhttps://wa.me/c/6289630985223', data.message)
        ...
        Client.cmd.on('calender', async (data) => {
            ...
            if(data.isQuotedImage || data.type == 'imageMessage') {
                ...
                Client.sendFileFromBase64(data.from, getAxios.data.toString('base64'), 'p.jpg', '*Gambar berhasil dibuat!*\n\nhttps://shiroweb-dashboard.221447.repl.co', data.message)
            } else if(data.mentionedJidList.length > 0) {
                ppUrl = await client.getProfilePicture(data.mentionedJidList[0])
                if(!ppUrl) return data.reply('Profile picture not found!\n\nhttps://wa.me/c/6289630985223')
                Client.sendFileFromUrl(data.from, `${configs.apiUrl}/api/calender?apikey=${configs.zeksKey}&image=${encodeURIComponent(ppUrl)}`, 'calender.jpg', '*Gambar berhasil dibuat!*\n\nhttps://shiroweb-dashboard.221447.repl.co', data.message)
        ...
        Client.cmd.on('removebg', async (data) => {
            ...
            if(data.isQuotedImage || data.type == 'imageMessage') {
                ...
                ppUrl = await client.getProfilePicture(data.mentionedJidList[0])
                if(!ppUrl) return data.reply('Profile picture not found!\n\nhttps://wa.me/c/6289630985223')
                Client.sendFileFromUrl(data.from, `${configs.apiUrl}/api/removebg?apikey=${configs.zeksKey}&image=${encodeURIComponent(ppUrl)}`, 'calender.jpg', '*Gambar berhasil dibuat!*\n\nhttps://shiroweb-dashboard.221447.repl.co', data.message)
            } else data.reply(`Wrong format!, tag someone or reply image with ${data.prefix}calender`)
        ...
        Client.cmd.on('drawing', async (data) => {
            ...
            if(data.isQuotedImage || data.type == 'imageMessage') {
                ...
                ppUrl = await client.getProfilePicture(data.mentionedJidList[0])
                if(!ppUrl) return data.reply('Profile picture not found!\n\nhttps://wa.me/c/6289630985223')
                Client.sendFileFromUrl(data.from, `${configs.apiUrl}/api/draw-image?apikey=${configs.zeksKey}&image=${encodeURIComponent(ppUrl)}`, 'calender.jpg', '*Gambar berhasil dibuat!*\n\nhttps://shiroweb-dashboard.221447.repl.co', data.message)
        ...
        Client.cmd.on('sketch', async (data) => {
            ...
            if(data.isQuotedImage || data.type == 'imageMessage') {
                ...
                ppUrl = await client.getProfilePicture(data.mentionedJidList[0])
                if(!ppUrl) return data.reply('Profile picture not found!\n\nhttps://wa.me/c/6289630985223')
                Client.sendFileFromUrl(data.from, `${configs.apiUrl}/api/sketch-image?apikey=${configs.zeksKey}&image=${encodeURIComponent(ppUrl)}`, 'calender.jpg', '*Gambar berhasil dibuat!*\n\nhttps://shiroweb-dashboard.221447.repl.co', data.message)
            } else data.reply(`Wrong format!, tag someone or reply image with ${data.prefix}drawing`)
        ...
        Client.cmd.on('*', async (data) => {
            ...
            switch(command.toLowerCase()) {
                ...
                case 'tomp3':
                    ...
                    convertMp3(dlfilez).then(data =>Client.sendFileFromUrl(from, data, 'p.mp3', '', message)).catch(er => Client.reply(from, 'server sedang error!\n\nhttps://wa.me/c/6289630985223', message))
					break
                case 'stikerwm':
                case 'stickerwm':
                case 'swm':
                ...
                case 'takestik':
                    ...
                    if(data.body == "") return data.reply(`Kirim perintah *${data.prefix}${data.command} [ pack|author ]*\nContoh : ${data.prefix}${data.command} punya|shiro`)
                    data.reply(mess.wait)
                    p = data.body
                    text = p.split('|')
                ...
                case 'sfire':
                    ...
                    } else data.reply(`format salah!, tag seseorang atau reaply image dengan ${data.prefix}stickerfire`)
                    break
                case 'stikernobg':
                case 'stickernobg':
                case 'snobg':
                ...
                case 'nulis':
                    ...
                    Client.sendFileFromUrl(from, `${configs.apiUrl}/api/${command}?text=${data.body}&apikey=${configs.zeksKey}`, 'gambar.jpg', `*Gambar berhasil dibuat!* @${data.sender.split('@')[0]}`, message)
                        ...
                        data.reply('server sedang error\n\nhttps://wa.me/c/6289630985223')
                    }
                    break
                case 'wolflogo':
                case 'logoaveng':
...
`
                ...
                case 'searchmusik':
                    ...
                    if(data.isQuotedAudio) {
                        ...
                        axios(`${configs.apiUrl}/api/searchmusic?apikey=${configs.zeksKey}`, {
                            ...
                            }).catch(() => Client.reply(from, 'Internal server sedang error!\n\nhttps://wa.me/c/6289630985223,', message))
                    } else Client.reply(from, 'format salah!\n\nhttps://shiroweb-dashboard.221447.repl.co', message)
                    break
                case 'wallpaper':
				    try{
                    if(isLimit(data.sender)) return data.reply(mess.limit)
                ...
                case 'hidetag':
                case 'everyone':
                    if(!isOwner) return data.reply('hanya bisa di gunakan oleh owner!\n\nhttps://wa.me/c/6289630985223')
                    var mention = []
                    data.groupMetadata.participants.forEach((member, i) => {
...
\njoin discord\nhttps://discord.gg/YzpFy77yB7\nfollow ig https://instagram.com/22.1447?utm_medium=copy_link`)
                    break
                    /*DLL*/
                case 'stickermenu':
                    ...
                    break
                case 'status':
                    data.reply('bot telah Online banh\n\n\nhttps://shiroweb-dashboard.221447.repl.co')
                    break
                case 'owner':
                    data.reply('ada apa sayang manggil owner ku 😙\n\nhttps://wa.me/c/6289630985223')
                    break
                case 'sewa':
                    data.reply('*hay kak*  😁 \n\n*Jika ingin menyewa bot berikut harga sewa bot nya*\n\n*_5 hari : 5k_*\n*_2 minggu : 10k_*\n*_2 bulan : 15k_*\n*tertarik ?*\n\n*chat owner jika tertarik no owner bisa di dapatkan dengan mengetik #owner*\n\n\nhttps://wa.me/c/6289630985223')
                    break
                case 'intro':
                    data.reply('ɪɴᴛʀᴏ\n┌ > ɴᴀᴍᴀ : \n┌ > ᴀsᴀʟ ᴋᴏᴛᴀ : \n┌ > ᴜsɪᴀ : \n┌ > ɢᴇɴᴅᴇʀ : \n┌ > sᴛᴀᴛᴜs :\nhttps://shiroweb-dashboard.221447.repl.co')
                    break                    
                ...
                case 'toimage':
                    ...
                    if(message.message.extendedTextMessage.contextInfo.quotedMessage.stickerMessage.isAnimated) {
                        axios(`https://serv-api.zeks.xyz/sticker/togif`, { method: "post", headers: { "content-type": 'application/json' }, data: {base64: mtdt.toString('base64')}}).then(bf => {
                            Client.sendFileFromBase64(from, bf.data.result, 'to.gif', 'BERHASIL!\n\nhttps://shiroweb-dashboard.221447.repl.co', message)
			})
                    } else {
                        axios(`https://api.zeks.me/sticker/png`, { method: "post", headers: { "content-type": 'application/json' }, data: { base64: mtdt.toString('base64')}}).then(bf => {
                            Client.sendFileFromBase64(from, bf.data.result, 'to.png', 'BERHASIL!\n\nhttps://shiroweb-dashboard.221447.repl.co', message)
                        })
                    }
                    break
            }
